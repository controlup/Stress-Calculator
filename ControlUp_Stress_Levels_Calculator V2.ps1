<#
.SYNOPSIS
        This script can be used to calculate suggested Stress thresholds for ControlUp Real-Time Console
.DESCRIPTION
        The ControlUp Stress Settings Calculator enables you to use your own data to determine how to set your Stress Thresholds in the Real-Time Console.
...
.PARAMETER yellowpercentile
        Number for the percentile to be used for recommending the Yellow Threshold
...
.EXAMPLE
        The tool can be used as an automated action or directly from powershell
        D:\temp\ControlUp_Stress_Levels_Calculator_v2.ps1 -sourcepath D:\dash_dataset\Dataset\ -yellowpercentile 75 -redpercentile 90 -filteredaccounts "Loginbot,wouterk" -ExportPath "d:\temp"
.NOTES
        Changelog:
            v2:
                Moved from monitor exports to export-cuquery exports
                Added culture difference fix
                Added ability to get folders for sessions and logical disks
                Added multiple types of metrics to be calculated
...
write-verbose -Message "Configured Yellow threshold percentile as $yellowpercentile "
write-verbose -Message "Configured Red threshold percentile as $redpercentile "
...
if ($null -ne $filteredaccounts) {
    ...
    $userfilterstatus = $True
}
else {
    $userfilterstatus = $False
    $Userfilter = New-Object System.Collections.ArrayList
}


$filteredmetrics = "Id,CapacityDeduplicationDatastoreType,NTPServers,IP"
...
write-verbose "Running from Powershell $psversion"


if (!($psmajorversion -eq 5) -AND !($psmajorversion -ge 7)) {
    throw "No powershell 5 or 7 found"
}
elseif (($psmajorversion -ge 7) -AND ($iswindows -ne $True)) {
    throw "This script runs only on Windows machines"
}

# Create arrays for both monitor and Insights related data
# $Monitor_Sourcedata=New-Object System.Collections.ArrayList
$Monitor_Sourcedata = [System.Collections.ArrayList]::Synchronized((New-Object System.Collections.ArrayList)) #for threadsafe foreach - parallel in ps7
                ...
                <ScrollViewer
                    ...
                    <Canvas Height="440" Width="950">
                        ...
                        >
                                ...
                                <ComboBoxItem Content="Datastores"/>
                                <ComboBoxItem Content="Datastores on Hosts"/>
                                <ComboBoxItem Content="Virtual Disks"/>
                                <ComboBoxItem Content="Logical Disks"/>
                                <ComboBoxItem Content="App Delivery Controllers"/>
                                <ComboBoxItem Content="Gateways"/>
                                <ComboBoxItem Content="Load Balancers"/>
                                <ComboBoxItem Content="LB ServiceGroups"/>
                                <ComboBoxItem Content="LB Services"/>
                        </ComboBox>
                        ...
                        >
                            <DataGrid.Columns>
                                ...
                                <DataGridTextColumn Header="Suggested Stress Threshold" Binding="{Binding Stress_Level}" Width="auto">
                                    <DataGridTextColumn.CellStyle>
                                        <Style TargetType="DataGridCell">
                        ...
                        />
                            ...
                            Content = 'Enable Custom Folders (Requires Reload)'
                            HorizontalAlignment="Left"
                            Height="60"
                            Width="300"
                            Margin="380,100,0,0"
                        ...
                        <RichTextBox
                            ...
                            <RichTextBox
                                ...
                                <FlowDocument>
                                    ...
                                    <Paragraph Margin="6" TextAlignment="Left"><Bold>Enable custom folders</Bold>
                                    <LineBreak/>
                                    Select to enable folders for Sessions and Logical Disk metrics. This requires at least one Computers Export and will slow down loading of the data.
                                    </Paragraph>
    ...
    elseif ($dontcalculate -eq $true) {
        ...
        try {
            if ($Monitor_Ignore_0_values.IsChecked -eq "True") {
                ...
                $datatocalculate = $filtereddatapoints.$metricname
                if ($datatocalculate -like "*,*") {
                    write-verbose "Values containing komma found. Switcheroo to period."
                    $datatocalculate = $datatocalculate -replace ",", "."
                }
                #write-verbose -message $filtereddatapoints.$metricname
                $yellow = Get-Percentile -Sequence $datatocalculate -Percentile $useforyellow
                $red = Get-Percentile -Sequence $datatocalculate -Percentile $useforred
                $average = [Linq.Enumerable]::average([decimal[]] @($datatocalculate))
                $median = (get-median $datatocalculate)
                $Monitor_Datapoints = $datatocalculate.count
            }
            else {
                $datatocalculate = $Monitor_Data.$metricname
                if ($datatocalculate -like "*,*") {
                    write-verbose "Values containing komma found. Switcheroo to period."
                    $datatocalculate = $datatocalculate -replace ",", "."
                }
...
function load-Monitor_data {
    ...
    [GC]::Collect()
    write-verbose "Cleaning up done"

    switch ($filetypetmp) {
        "Virtual Disks" { $filetype = "vDisks" }
        "Logical Disks" { $filetype = "LogicalDisks" }
        "Load Balancers" { $filetype = "LoadBalancers" }
        "LB ServiceGroups" { $filetype = "LBServiceGroups" }
        "LB Services" { $filetype = "LBServices" }
        "App Delivery Controllers" { $filetype = "NetScalers" }
        default { $filetype = $filetypetmp }
    }
    ...
    elseif (!(test-path $sourcefolder)) {
        ...
        if ($files.count -gt 0) {
            # $header=((get-childitem $sourcefolder).where({$_.name -like "$filetype*"}) | select-object -first 1 | get-content | Select-Object -first 1).Split(",")
            if ($psmajorversion -eq 5) {
                write-verbose "Running in slower Powershell 5 mode"
                foreach ($file in $files) {
                    $content = $null
                    if ($filetype -eq "sessions") {
                        [array]$content = (import-Csv -path $file.fullname).where({ $_.sUserName -ne "" -and $_.sSessionName -ne "Services" })
                        if ($Userfilter) {
                            [array]$content = $content.where({ $_.sUserName | select-string -pattern $userfilter -notmatch })
                        }
                    }
                    else {
                        [array]$content = import-csv -path $file.fullname
                    }
                    $content.foreach({ [void]$Monitor_Sourcedata.add($_) })
                }
            }
            elseif ($psmajorversion -ge 7) {
                write-verbose "Running in faster Powershell 7 mode"
                $files | foreach-object -Parallel {
                    $content = $null
                    if ($using:filetype -eq "sessions") {
                        [array]$content = (import-Csv -path $_.fullname).where({ $_.sUserName -ne "" -and $_.sSessionName -ne "Services" })
                        if ((($using:Userfilterstatus) -eq $True)) {
                            $filtered_users = $using:Userfilter
                            [array]$content = $content.where({ $_.sUserName | select-string -pattern ($filtered_users) -notmatch })
                        }
                    }
                    else {
                        [array]$content = import-csv -path $_.fullname
                    }
                    $content.foreach({ [void]($using:Monitor_Sourcedata).add($_) })
                }
            }
            else {
                write-host "Powershell version is not 5 or greater than 7, exiting."
                exit
            }

            $header = $Monitor_Sourcedata | get-member -type NoteProperty | select-object -ExpandProperty Name
            $stopwatch.stop()
            ...
            # $usedmetrics=@()
            foreach ($i in $header) {
                # $p=$Monitor_Sourcedata."$i" | where-object {$_ -ne ""} | select-object -first 100
                try {
                    # get-median $p | out-null
                    # Get-Percentile -sequence $p -Percentile 0.75 | out-null
                    [void] $Monitor_Available_Metrics.Items.Add($i)
                    # $usedmetrics+=$i
                    # write-verbose "Adding $i to available Metrics."
                }
                catch {
                    # $filteredmetrics+=$i
                    # write-verbose "Error calculating $i, adding to Metric filter."
                }
            }
            # $filteredmetrics | out-file -path "d:\filteredmetrics_$filetype.txt"
            # $usedmetrics | out-file -path "d:\usedmetrics_$filetype.txt"
            $stopwatch.stop()
            $elapsedtime = $stopwatch.Elapsed.TotalSeconds
            write-verbose -message "Checking the metrics took $elapsedtime seconds"
            $stopwatch = [System.Diagnostics.Stopwatch]::StartNew()
            if ($header -contains "ParentFolderPath") {
                $Monitor_Folder_Selector.Visibility = "Visible"
                $Monitor_SourceData | Add-Member -MemberType AliasProperty -name "Folder Path" -value ParentFolderPath
                ...
                $Monitor_Folder_Selector.Text = "All Folders"
            }
            elseif ($header -contains "Path") {
                $Monitor_Folder_Selector.Visibility = "Visible"
                $Monitor_SourceData | Add-Member -MemberType AliasProperty -name "Folder Path" -value Path
                $foldermenuitems = $Monitor_Sourcedata."Folder path" | select-object -unique
                $foldermenuitems | sort-object | foreach-object { [void] $Monitor_Folder_Selector.Items.Add($_) }
                [void] $Monitor_Folder_Selector.Items.Add("All Folders")
                $Monitor_Folder_Selector.Text = "All Folders"
            }
            elseif ($header -contains "hFolderPath" -and $filetype -eq "Hosts") {
                $Monitor_Folder_Selector.Visibility = "Visible"
                $Monitor_SourceData | Add-Member -MemberType AliasProperty -name "Folder Path" -value hFolderPath
                $foldermenuitems = $Monitor_Sourcedata."Folder path" | select-object -unique
                $foldermenuitems | sort-object | foreach-object { [void] $Monitor_Folder_Selector.Items.Add($_) }
                [void] $Monitor_Folder_Selector.Items.Add("All Folders")
                $Monitor_Folder_Selector.Text = "All Folders"
            }
            elseif ($header -contains "hFolderPath" -and $filetype -eq "Datastores") {
                $Monitor_Folder_Selector.Visibility = "Visible"
                $Monitor_SourceData | Add-Member -MemberType AliasProperty -name "Folder Path" -value hFolderPath
                $foldermenuitems = $Monitor_Sourcedata."Folder path" | select-object -unique
                $foldermenuitems | sort-object | foreach-object { [void] $Monitor_Folder_Selector.Items.Add($_) }
            ...
            elseif ($header -contains "hFolderPath" -and $filetype -eq "Datastores on Hosts") {
                $Monitor_Folder_Selector.Visibility = "Visible"
                $Monitor_SourceData | Add-Member -MemberType AliasProperty -name "Folder Path" -value hFolderPath
                $foldermenuitems = $Monitor_Sourcedata."Folder path" | select-object -unique
                $foldermenuitems | sort-object | foreach-object { [void] $Monitor_Folder_Selector.Items.Add($_) }
            ...
            elseif ($filetype -eq "vDisk") {
                $Monitor_Folder_Selector.Visibility = "Hidden"
            }

            if ($Enable_Folders_For_Sessions_LogicalDisks.IsChecked -eq "True" -and ($filetype -eq "Sessions" -or $filetype -eq "LogicalDisks")) {
                $computerdata = (get-childitem $sourcefolder).where({ $_.name -like "*Computers*" }) | sort-object LastWriteTime | select-object -last 1 | get-content | convertfrom-csv
                if ($null -eq $computerdata) {
                    $ButtonType = [System.Windows.Forms.MessageBoxButtons]::OK
                    $MessageIcon = [System.Windows.Forms.MessageBoxIcon]::Information
                    $MessageBody = "please make sure a recent Computers Export is available as this is used to get the folder structure for this metric, disabling folder views."
                    $MessageTitle = "Warning"
                    [System.Windows.Forms.MessageBox]::Show($MessageBody, $MessageTitle, $ButtonType, $MessageIcon)
                    $Monitor_Folder_Selector.Visibility = "Hidden"
                    $Monitor_SourceData.foreach{ [void]$Monitor_Data.add($_) }
                    ...
                    $Monitor_SourceData.foreach{
                        if ($filetype -eq "Sessions") {
                            $scid = $_.ComputerId
                        }
                        elseif ($filetype -eq "LogicalDisks") {
                            $scid = $_.ComputerGuid
                        }
                        if ($scid) {
                            $computer = $computerdata | where-object { $_.ObjectGuid -eq $scid } | select-object -first 1
                            if ($computer) {
                                $folderpath = $computer.ParentFolderPath
                                $_ | Add-Member -NotePropertyName "Folder Path" -NotePropertyValue $folderpath
                            }
                            else {
                                write-verbose "Related Computer not found"
                            }
                        }
                        else {
                            $FQDN = $_.FQDN
                            write-verbose "No Related ComputerId found for $FQDN"
                        }
                        [void]$Monitor_Data.add($_)
                    }
                    $Monitor_Folder_Selector.Visibility = "Visible"
                    $foldermenuitems = $Monitor_Sourcedata."Folder path" | select-object -unique
                    $foldermenuitems | sort-object | foreach-object { [void] $Monitor_Folder_Selector.Items.Add($_) }
...
$Enable_Folders_For_Sessions_LogicalDisks.Add_Checked({ Monitor-clear_selected_metric })
$Enable_Folders_For_Sessions_LogicalDisks.Add_Unchecked({ Monitor-clear_selected_metric })
...
#region Help actions
#$Help_Link1.Add_MouseLeftButtonUp({start-process 'https://support.controlup.com/docs/stress-levels-calculator-tool'})
$Help_Link1.Add_MouseLeftButtonUp({ start-process 'https://support.controlup.com/docs/stress-level-calculator-tool-v2' })
#endregion
